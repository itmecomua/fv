...
<tr>
    ...
    <th style="text-align:center;">
        <u>
            <a href="javascript:void(0);" onclick="window.doSort('{$manager->getConst('F_IS_SHOW_PROMO')}','{if $order.direct=='asc'}desc{else}asc{/if}'); return false;">
                Отображать в промо{if $order.field==$manager->getConst('F_IS_SHOW_PROMO')}<img src="{$fvConfig->get('dir_web_root')}img/{$order.direct}_arrow.gif" width="11" height="13">{/if}
            </a>
        </u>            
    </th>    
...
<tr>
    ...
    <td class="mixed">{if $inst->IsShowPromo()}Да{else}Нет{/if}</td>
    <td>
