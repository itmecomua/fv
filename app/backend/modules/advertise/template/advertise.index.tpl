        ...
        <form id="filter" method="post" action="/backend/{$module}" onsubmit="return true;">
            <div class="form">
                <fieldset>
                    ...
                    <label>Тип</label>                    
                    {html_options options=$manager->getListType(null,"Любой")
                          name="search[type_id]"
                          selected=$search.type_id}
                </fieldset>
                <div class="operation">
...
<div class="table_body">
    <table class="text">
        ...
        {if $List->getElementCount() > 0}
            ...
            <tr>            
                <td class="mixed">{$ex->getTypeName()}</td>
                <td class="mixed">{$ex->getName()}</td>
                <td class="mixed">{$ex->getText()}</td>                        
                <td class="mixed">                    
                    {if $ex->getImageSrc()}
            ...
            {/foreach}
        {else}
            <tr><td colspan="7">Пусто. Для добавления используйте кнопку "Добавить"</td></tr>
        {/if}
    </table>
