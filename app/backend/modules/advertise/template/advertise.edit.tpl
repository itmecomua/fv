...
<FORM method="post" action="/backend/{$module}/save/">
    <div class="form" style="padding-top:10px;">
        ...
        <label>Тип</label>            
            {if $ex->isNew()}
            {html_options options=$manager->getListType()
                          name="m[type_id]"
                          selected=$ex->getType()}
            {else}
                <select disabled="disabled"><option>{$manager->getListType($ex->getType())}</option></select>
                <input type="hidden" name="m[$ex->getType()]" />
            {/if}
        <br />
        <label>URL:</label>
        <input type="text" name="m[url]" value="{$ex->getURL()}" id="url" />        
        <br />
        <label>Открыть в новом окне</label>
        <input type="radio" name="m[target_blank]" value="1" {if $ex->isTargetBlank()}checked="checked" {/if}>Да
        <input type="radio" name="m[target_blank]" value="0" {if !$ex->isTargetBlank()}checked="checked" {/if}>Нет
        <br />
        <br />
        ...
        <br />
        <label>Текст</label>
        <textarea name="m[text]">{$ex->getText()|escape}</textarea>
        <div style="clear: both;">&nbsp;</div>
