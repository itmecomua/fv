...
<tr>
    ...
    <th style="text-align:center;">
        <u>
            <a href="javascript:void(0);" onclick="window.doSort('{$manager->getConst('F_COUNTRY')}','{if $order.direct=='asc'}desc{else}asc{/if}'); return false;">
                Страна{if $order.field==$manager->getConst('F_COUNTRY')}<img src="{$fvConfig->get('dir_web_root')}img/{$order.direct}_arrow.gif" width="11" height="13">{/if}
            </a>
        </u>            
    </th>    
    <th style="text-align:center;">
        <u>
            <a href="javascript:void(0);" onclick="window.doSort('{$manager->getConst('F_RESORT')}','{if $order.direct=='asc'}desc{else}asc{/if}'); return false;">
                Курорт{if $order.field==$manager->getConst('F_RESORT')}<img src="{$fvConfig->get('dir_web_root')}img/{$order.direct}_arrow.gif" width="11" height="13">{/if}
            </a>
        </u>            
    </th>
    <th style="text-align:center;">
        <u>
            <a href="javascript:void(0);" onclick="window.doSort('{$manager->getConst('F_HOTEL_TYPE')}','{if $order.direct=='asc'}desc{else}asc{/if}'); return false;">
                Тип отеля{if $order.field==$manager->getConst('F_HOTEL_TYPE')}<img src="{$fvConfig->get('dir_web_root')}img/{$order.direct}_arrow.gif" width="11" height="13">{/if}
            </a>
        </u>            
    </th>
    <th>&nbsp;</th>
...
<tr>
    ...
    <td class="mixed">{$inst->getCountry()->getName()}</td>
    <td class="mixed">{$inst->getResort()->getName()}</td>
    <td class="mixed">{$inst->getHotelType()->getName()}</td>
    <td>
        ...
        <a href="javascript: void(0);" onclick="if (confirm('Вы действительно желаете удалить страницу?')) go('{$inst->getBackendDeleteURL()}'); return false;">
