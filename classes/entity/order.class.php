...
class Order extends Dictionary
{
    ...
    {
        $valid = $this->doValidateEmpty($value);
        $this->setValidationResult("country",$valid,"Это поле обязательное");
        return $valid;
    }
    public function validateCity($value)
    {
        $valid = $this->doValidateEmpty($value);
        $this->setValidationResult("city",$valid,"Это поле обязательное");
        return $valid;
    }
    ...
    public function doSendEmail()
    {
        ...
        $mail = new Mail();
        $data = array();
        $data []= "Получен новый заказ [" . date("Y-m-d H:i") . "]";
        $data []= "Имя: {$this->name}";
