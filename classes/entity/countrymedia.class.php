<?php
/**
* Сущность медиа для страны                                                            
* 
*/
class CountryMedia extends Dictionary
{
    ...
    public function validateType_id($value)
    {
        $valid = $this->isNew() || in_array($value,CountryMediaManager::getInstance()->getListMediaType());
        $this->setValidationResult("type_id",$valid,"Необходимо указать тип изображения");
        return $valid;
    }
    // }} 
    ...
    */
    ...
    {
        $web = $isWeb ? "_web" : "";              
        $path = fvSite::$fvConfig->get('path.upload.mediapharmacy'.$web);
    ...
    */    
    ...
    {        
        $tmpDir = CountryMediaManager::getInstance()->getTempImageFolder();
        ...
        if(file_exists( $tempImage ))
        {
            $arrType = array(fvUploaded::IMAGE_TYPE_THUMB,
                             ...
                             fvUploaded::IMAGE_TYPE_SMALLONE,
                             fvUploaded::IMAGE_TYPE_SMALLTWO,
    ...
    */    
    ...
    {
        $web = $web ? "_web" : "";        
        $destPath = fvSite::$fvConfig->get("path.upload.mediacountry{$web}");        
        $folder = $destPath . str_pad($this->country_id, 8, 0, STR_PAD_LEFT ) ."/";
    ...
    */
    ...
    /**
    * Получить тип медиа в строку
    *
    *  @return string
    */
    public function getTypeToStr()
    {
        return CountryMediaManager::getInstance()->getListMediaType($this->type_id);
    }
    /**
    * Получить тип медиа
    *
    *  @return int тип медиа
    */
