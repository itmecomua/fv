...
class fvActionFilter implements iFilter {
    ...
    public function execute() {        
        ...
        $LayoultClass =  fv::getFvConfig()->get("layoult");		
		...
		*/
        ...
        if (($action = fvDispatcher::getInstance()->getModule(fvRoute::getInstance()->getModuleName(), 'action')) === false) {
            fvDispatcher::getInstance()->redirect(fv::getFvConfig()->get('page_404', 0, 404));
        }
        ...
        switch ($result) 
        {
            ...
            case fvAction::$FV_ERROR:
                fvDispatcher::getInstance()->redirect(fv::getFvConfig()->get('error_page', 0, 404));
                break;
